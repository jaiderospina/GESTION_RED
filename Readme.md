...
## 7. Integración de Windows 11 mediante SNMP

Esta sección incorpora un equipo **Windows 11 24H2** a Zabbix mediante el servicio SNMP opcional de Windows, sin instalar el agente nativo de Zabbix. El servidor origina las consultas hacia **UDP/161** y Windows devuelve las respuestas del agente.

> **Objetivo:** monitorizar únicamente equipos autorizados, con una comunidad propia, permisos de solo lectura y acceso restringido. SNMPv2c transmite la comunidad sin cifrar; para entornos no confiables se recomienda SNMPv3.

### 7.1 Entorno de referencia

| Elemento | Valor de laboratorio |
|---|---|
| Servidor | Zabbix 7.4.13 sobre CentOS Stream 10 |
| Equipo | Windows 11 24H2 |
...
### 7.2 Arquitectura y flujo

```mermaid
flowchart LR
    Z[Servidor Zabbix] -- "SNMP GET · UDP 161" --> W[Windows 11]
    W -- "Respuesta SNMPv2c" --> Z
    F[Firewall Windows] -. "Permite UDP 161" .-> W
```

La regla principal se configura en Windows porque las consultas entran al puerto UDP/161 del equipo monitorizado.

### 7.3 Preparar el servidor Zabbix
...
La salida debe incluir una linea similar a `snmp poller #1`.

### 7.4 Instalar SNMP en Windows 11

Abre PowerShell con **Ejecutar como administrador**:

```powershell
Get-WindowsCapability -Online -Name "SNMP*"
$capability = Get-WindowsCapabiliity -Online -Name "SNMP*" | Where-Object Name -like "SNMP.Client*" | Select-Object -First 1
if (-not $capability) { throw "No se encontró SNMP.Client" }
if ($capability.State -ne "Installed") { Add-WindowsCapability -Online -Name $capability.Name }
Set-Service -Name SNMP -StartupType Automatic
Start-Service -Name SNMP
Get-Service -Name SNMP | Select Name,Status,StartType
```

El resultado esperado es **Running / Automatic**. `SMMp` responde consultas; `SNMPTrap` utiliza para recibir traps y no lo sustituye.

### 7.5 Configurar comunidad y agente

En `services.msc`, abre **Servicio SNMP → Propiedades → Seguridad**. Agrega `zbx_monitor` con derechos **SOLO LECTURA**. Durante la prueba acepta cualquier host de forma temporal. En **Agente**, marca **Físico**, **Aplicaciones**, **Vínculo de datos y subred**, **Internet** y **De extremo a extremo**.

```powershell
Restart-Service -Name SNMP
```

La comunidad debe ser distinta de `public`, distingue mayúsculas y minúsculas y nunca debe tener permisos de escritura.

### 7.6 Verificar el firewall

```powershell
Get-NetFirewallRule -DisplayName "*SNMP*" | Select DisplayName,Enabled,Profile
New-NetFirewallRule -DisplayName "SNMP-In-UDP161" -Direction Inbound -Protocol UDP -LocalPort 161 -Action Allow -Profile Any -Enabled True
```

### 7.7 Validar con `snmpwalk`
...
Comprueba `sysName`, `sysUpTime` y `sysServices`. Con los cinco servicios habilitados, `sysServices` suele devolver `79`. No continúes al alta en Zabbix hasta obtener respuesta.

### 7.8 Crear el host en Zabbix

En **Data collection → Hosts → Create host** utiliza:

| Campo | Valor |
|---|---|
| Host name | `Win11-SNMP` |
| Host groups | `Windows servers` |
| Template | `Windows by SNMP` |
| Interface | `SNMP`, no `Zabbix agent` |
| IP / Port | `172.17.47.225 : 161` |
| SNMP version | `SNMPv2` |
| Community | `{$SNMP_COMMUNITY}` |

En **Macros** define `{$SNMP_COMMUNITY}` con el valor `zbx_monitor`. Guarda y espera entre dos y tres minutos; el indicador SNMP debe quedar verde.

### 7.9 Restringir el acceso

Vuelve a |Servicio SNMP → Seguridad*, selecciona **Aceptar paquetes SNMP de estos hosts**, agrega la IP real del servidor Zabbix y reinicia:

```powershell
Restart-Service -Name SNMP
```

Valida nuevamente `snmpwalk`. Si falla después de restringir el origen, revisa NAT o virtualización: la IP observada por Windows puede ser diferente de la IP lógica del servidor.

### 7.10 Diagnóstico y lista final

| Síntoma | Comprobación |
|---|---|
| Timeout | Servicio, comunidad, UDP/161, firewall, ruta y conectividad |
| `sysServices` reducido | Cinco servicios del agente y reinicio |
| Zabbix sin datos | Interfaz SNMP, plantilla y macro |
| Falla tras restringir | IP de origen real, NAT o red virtual |

- [ ] Herramientas Net-SNMP instaladas.
- [ ] Servicio SNMP automático y en ejecución.
- [ ] Comunidad propia con solo lectura.
- [ ] UDP/161 permitido.
- [ ] `snmpwalk` devuelve `sysName` y `sysUpTime`.
- [ ] Host configurado con `Windows by SNMP`.
- [ ] Macro `{$SNMP_COMMUNITY}` definida.
- [ ] Acceso limitado a la IP del servidor.
> **Privacidad:** no recopiles pulsaciones, archivos privados, capturas de pantalla ni datos no necesarios para el objetivo tücnico autorizado. No publiques comunidades, contraseüas, tokens, logs ni direcciones internas.
