...
## 8. Actualización: monitorización de Windows 11 mediante SNMP

Esta actualización incorpora un equipo **Windows 11 24H2** a **Zabbix 7.4.13** utilizando el servicio SNMP de Windows, sin instalar el agente nativo de Zabbix. La implementación se realizó sobre un servidor **CentOS 10** y quedó operativa y verificada.

> **Resultado:** el host `Win11-SNMP` quedó registrado con la plantilla `Windows by SNMP`, interfaz SNMPv2c y **68 elementos recolectando datos**.

### 8.1 Entorno de la implementación

| Componente | Valor |
|---|---|
| Servidor Zabbix | `192.168.159.129` |
| Equipo Windows | `DESKTOP-FK5S8HJ` |
| Dirección Windows | `172.17.47.225` |
| Protocolo | SNMPv2c sobre UDP/161 |
| Comunidad | `zbx_monitor` |
| Permisos | Solo lectura |
| Plantilla | `Windows by SNMP` |

El servidor Zabbix origina las consultas hacia UDP/161 del equipo Windows. Por ello, el firewall crítico se configura en Windows como tráfico entrante. La comunicación atravesó NAT de VMware, por lo que la dirección de origen observada debe confirmarse antes de restringir el agente.

### 8.2 Preparar el servidor

Comprueba que Zabbix tenga soporte SNMP e instala las herramientas de diagnóstico:
...
La presencia del proceso `snmp poller` confirma que el servidor puede ejecutar comprobaciones SNMP. `net-snmp-utils` permite validar el agente antes de involucrar la configuración de Zabbix.

### 8.3 Instalar SNMP en Windows 11

Abre PowerShell con **Ejecutar como administrador** y consulta la característica opcional:

```powershell
Get-WindowsCapability -Online -Name "SNMP*"
```

Instala el componente `SNMP.Client`:

```powershell
Add-WindowsCapability -Online -Name "SNMP.Client~~~~0.0.1.0"
```

![Instalación de la característica SNMP en Windows](instalacion-capability.png)

Comprueba que el servicio esté activo y configurado para iniciar automáticamente:
...
![Servicio SNMP configurado en Windows](servicio-snmp.png)

`SNMP` es el servicio que responde las consultas. `SNMPTrap` es un servicio diferente para recepción de traps y no sustituye al agente SNMP.

### 8.4 Configurar comunidad y servicios del agente

Desde `services.msc`, abre las propiedades de **Servicio SNMP**. En la pestaña **Seguridad**, agrega la comunidad `zbx_monitor` con permisos **SOLO LECTURA**. Durante la validación inicial se puede aceptar cualquier host de forma temporal.

![Configuración de la comunidad SNMP](configuracion-seguridad-snmp.png)

En la pestaña **Agente**, activa los cinco servicios disponibles: Físico, Aplicaciones, Vínculo de datos y subred, Internet y De extremo a extremo. Estas opciones habilitan ramas adicionales del árbol de OIDs.

![Configuración de seguridad del agente SNMP](seguridad-comunidad.png)
...
![Reinicio y aplicación de la configuración SNMP](instalacion-windows.png)

### 8.5 Verificar el firewall

Comprueba las reglas creadas por Windows:

```powershell
Get-NetFirewallRule -DisplayName "*SNMP*" | Select DisplayName,Enabled,Profile
```

Si no existe una regla de entrada para UDP/161, créala:

```powershell
New-NetFirewallRule -DisplayName "SNMP-In-UDP161" `
  -Direction Inbound -Protocol UDP -LocalPort 161 -Action Allow -Profile Any
```

### 8.6 Validar el agente con `snmpwalk`

La consulta se ejecuta desde el servidor Linux, no desde Windows:

```bash
snmpwalk -v2c -c zbx_monitor 172.17.47.225 .1.3.6.1.2.1.1
```

La respuesta debe confirmar la identidad del equipo, los servicios activos y el tiempo desde el reinicio del agente.

![Consulta SNMP de validación desde el servidor](validacion-snmp.png)

![Resultado de la consulta snmpwalk](consulta-snmpwalk.png)

Si la consulta expira, revisa en este orden el servicio SNMP, la comunidad, la autorización temporal del origen, el firewall y la conectividad.

### 8.7 Registrar el host en Zabbix

Entra en **Recopilación de datos → Equipos → Crear equipo**. Configura la interfaz como **SNMP**, no como Agente, y utiliza la plantilla `Windows by SNMP`.

![Formulario de alta del host SNMP](alta-host-snmp.png)

Selecciona la plantilla específica para SNMP:

![Selección de la plantilla Windows by SNMP](seleccion-plantilla.png)

La interfaz debe utilizar la comunidad como macro:

```text
Comunidad SNMP: {$SNMP_COMMUNITY}
```

![Configuración de la interfaz SNMP](configuracion-host.png)

En la pestaña **Macros** del equipo define:

| Macro | Valor |
|---|---|
| `{$SNMP_COMMUNITY}` | `zbx_monitor` |

![Definición de la macro de comunidad](macro-comunidad.png)

La macro es necesaria porque, si se omite, la plantilla puede heredar `public` y los elementos fallarán aunque `snmpwalk` funcione.

### 8.8 Comprobar la operación

Después de guardar el equipo, espera entre dos y tres minutos y verifica que el indicador SNMP aparece en verde. La implementación obtuvo 68 elementos recolectando datos.

![Panel de Zabbix con el host SNMP configurado](panel-zabbix.png)

![Resultado final de la monitorización SNMP](resultado-zabbix.png)

También puedes revisar el estado final del equipo desde la vista de monitorización:

![Validación final del host Windows mediante SNMP](validacion-final.png)

### 8.9 Restringir el acceso al agente

Una vez confirmada la recolección, reemplaza **Aceptar paquetes SNMP de cualquier host** por **Aceptar paquetes SNMP de estos hosts**. Añade únicamente el origen real del servidor Zabbix. En el escenario documentado se consideraron `192.168.159.129` y la dirección resultante del NAT de VMware.

Reinicia el servicio y valida nuevamente:
...
![Validación de la restricción de acceso SNMP](validacion-final.png)

### 8.10 Incidencias y mantenimiento

| Síntoma | Causa | Solución |
|---|---|---|
| Se requiere elevación | PowerShell no se abrió como administrador. | Abrir una consola elevada. |
| Instalación aparentemente detenida | Consola en modo de selección de texto. | Pulsar `Esc` y verificar desde otra consola. |
| Solo aparece `SNMPTrap` | `SNMP.Client` no está instalado. | Instalar la característica y comprobar `Get-Service SNMP`. |
| `snmpwalk` no se reconoce | Se ejecutó en Windows. | Ejecutarlo en CentOS con `net-snmp-utils`. |
| No aparecen datos en Zabbix | Macro o comunidad incorrecta. | Definir `{$SNMP_COMMUNITY}` como `zbx_monitor`. |
| Falla después de restringir hosts | El NAT cambia la dirección de origen. | Autorizar el origen real o capturar UDP/161. |

SNMPv2c transmite la comunidad sin cifrado. La comunidad no predeterminada, los permisos de solo lectura y la restricción por dirección reducen la exposición, pero no eliminan el riesgo de captura. El agente SNMP de Windows ofrece menos métricas que el agente nativo de Zabbix y no es la opción adecuada para monitorizar con detalle servicios, procesos, eventos o comprobaciones personalizadas.

Microsoft considera SNMP una característica obsoleta; por ello, esta integración debe revisarse ante futuras versiones de Windows y compararse con alternativas basadas en CIM/WMI o con el agente nativo de Zabbix.

---

*Actualización basada en el procedimiento ejecutado y verificado el 2 de septiembre de 2026.*
